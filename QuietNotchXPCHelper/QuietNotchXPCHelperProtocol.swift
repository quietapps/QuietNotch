...
@objc protocol QuietNotchXPCHelperProtocol {
    ...
    // Dismiss the currently-visible macOS native OSD (brightness / volume
    // bezel HUD) by killing OSDUIHelper. The process auto-respawns on next
    // use, so this is effectively just a "hide what's on screen now" call.
...
/*
 To use the service from an application or other process, use NSXPCConnection to establish a connection to the service by doing something like this:
     ...
     connectionToService = NSXPCConnection(serviceName: "app.quiet.QuietNotch.QuietNotchXPCHelper")
     connectionToService.remoteObjectInterface = NSXPCInterface(with: (any QuietNotchXPCHelperProtocol).self)
     connectionToService.resume()

 Once you have a connection to the service, you can use it like this:

     if let proxy = connectionToService.remoteObjectProxy as? QuietNotchXPCHelperProtocol {
