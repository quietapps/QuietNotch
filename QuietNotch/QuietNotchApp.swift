...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Quiet Notch", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: QuietViewModel] = [:] // UUID -> QuietViewModel
    var window: NSWindow?
    let vm: QuietViewModel = .init()
    @ObservedObject var coordinator = QuietViewCoordinator.shared
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? QuietNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? QuietNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createQuietNotchWindow(for screen: NSScreen, with viewModel: QuietViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = QuietNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createQuietNotchWindow(
    ...
    func playWelcomeSound() {
        ...
        audioPlayer.play(fileName: "quiet", fileExtension: "m4a")
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = QuietViewModel(screenUUID: uuid)
                    let window = createQuietNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createQuietNotchWindow(for: selectedScreen, with: vm)
            }
