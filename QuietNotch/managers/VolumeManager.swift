...
final class VolumeManager: NSObject, ObservableObject {
    ...
    @MainActor func increase(stepDivisor: Float = 1.0) {
        ...
        setAbsolute(target)
        QuietViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(target))
    }
    ...
    @MainActor func toggleMuteAction() {
        ...
        toggleMuteInternal()
        QuietViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(willBeMuted ? 0 : resultingVolume))
    }
